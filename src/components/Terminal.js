...
import SpinningEarth from './EarthCard';
...
const Terminal = () => {
  ...
  const availableCommands = useMemo(() => [
    'help', 'gravity', 'g', 's', 'github', 'gh', 'discord', 'ds', 'email', 'em',
    'linkedin', 'li', 'ascii-selfie', 'population', 'p',
    'blast', 'b', 'clear', 'c', 'why', 'w', 'wiki', 'wikipedia',
    ...
    'password-generator', 'github-feed', 'earth', 'e',
    'gravity-inc', 'gravity-dec', 'gravity-invert', 'gravity-max', 'gravity-randomize', 'gravity-zero',
    'gi', 'gd', 'ginv', 'gmax', 'grandomize', 'gzero'
...
`
  ...
  const handleCommand = useCallback((command) => {
    ...
    switch (cmd) {
      ...
      case 'github':
      case 'gh':
        window.open('https://github.com/GithubAnant', '_blank');
      ...
      case 'email':
      case 'em':
        window.open('mailto:anantsinghal444@gmail.com', '_blank');
        addToOutput({ type: 'output', content: 'Opening email client...(mailto:anantsinghal444@gmail.com)' });
        break;
      case 'linkedin':
      case 'li':
        window.open('https://www.linkedin.com/in/anantsinghal1/', '_blank');
      ...
      case 'p':
        ...
        addToOutput({ type: 'output', content: 'Current Population: 8,245,947,592' });
        // Re-enable autoscroll shortly after render settles
        setTimeout(() => { suppressAutoScrollRef.current = false; }, 600);
        break;
        case 'blast':
          case 'b':
        addToOutput({ type: 'component', content: <SpinningEarth /> });
        addToOutput({ type: 'output', content: 'Current Population: 0' });
        break;
        case 'clear':
          case 'c':
            setOutput([]);
            ...
            case 'gravity':
              case 'g':
                ...
                case 'gravity-inc':
                  case 'gi':
                    addToOutput({ type: 'output', content: 'Gravity Increased' });
                    addToOutput({ type: 'output', content: 'Current Population: 1,531,388,021' });
                    break;
                    case 'gravity-dec':
                      case 'gd':
                        addToOutput({ type: 'output', content: 'Gravity Decreased' });
                        addToOutput({ type: 'output', content: 'Current Population: 3,546,955,233' });
                        break;
                        case 'gravity-invert':
                          case 'ginv':
                            addToOutput({ type: 'output', content: 'Gravity Inverted' });
                            addToOutput({ type: 'output', content: 'Current Population: 233' });
                            break;
                            case 'gravity-randomize':
                              case 'grandomize':
                                addToOutput({ type: 'output', content: 'Gravity Randomized' });
                                addToOutput({ type: 'output', content: 'Current Population: 499,233,740' });
                                break;
                                case 'gravity-max':
                                  case 'gmax':
                                    addToOutput({ type: 'output', content: 'Gravity Maxed' });
                                    addToOutput({ type: 'output', content: 'Current Population: 0' });
                                    break;
                                    case 'gravity-zero':
                                      case 'gzero':
                                        addToOutput({ type: 'output', content: 'Gravity Zeroed' });
                                        addToOutput({ type: 'output', content: 'Current Population: ??' });
        break;
      case 'help':
      ...
      case 'miscellaneous':
        ...
        case 'resume':
          case 'cv':
            ...
            addToOutput({ type: 'output', content: 'Coming Soon' });
      ...
      case 'google':
        ...
        case 'why':
        case 'w':
          addToOutput({ type: 'component', content: <WhyCard /> });
          break;
        case 'earth':
        case 'e':
          addToOutput({ type: 'component', content: <SpinningEarth /> });
      ...
      case 'hackermode':
        ...
        addToOutput({ type: 'output', content: `Nice try Kuber ;)` });
        break;
              case 'calculator':
        addToOutput({ type: 'component', content: (
  ...
  useEffect(() => {
    ...
    ${isMobile ? banners.small : banners.large}
      ...
      <p>Earth Gravity Patch Notes 2.0 
  ...
  return (
    ...
    <div key={index}>
        {item.type === 'input' ? (
            <div>
                <span className="ownerTerminal"><b>anant@earth</b></span>
                <b>:~$</b> {item.content}
            </div>
        ) : item.type === 'component' ? (
